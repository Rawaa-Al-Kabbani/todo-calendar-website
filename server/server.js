...
async function getListsByUser(userid) {
  return await makeQuery(
    "SELECT list_text, listid FROM todo_list WHERE userid=?",
    [userid]
  );
}
...
async function markTodoAsDone(userid, todoid) {
  return await makeQuery(
    "UPDATE todo_item SET done=True WHERE userid=? and todoid=?",
    [userid, todoid]
  );
}

async function deleteTodo(userid, todoid) {
  return await makeQuery("DELETE FROM todo_item WHERE userid=? AND todoid =?", [
    userid,
    todoid,
  ]);
}
...
async function editTodo(title, newStart, newEnd, list, tag, todoid, userid) {
  return await makeQuery(
    "UPDATE todo_item SET title = ?, start = ?, end = ?, listid = ?, tagid = ? WHERE todoid = ? AND userid = ?",
    [title, newStart, newEnd, list, tag, todoid, userid]
  );
}
...
async function getUsername(userid) {
  const userName = await makeQuery(
    "SELECT username FROM todo_db.user where user.userid=?",
    [userid]
  );
